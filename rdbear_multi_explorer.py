...
instrument BEFORE building bots on it, across THREE contract families with
durations from 2 up to 60 minutes: NOTOUCH, EXPIRYRANGE ("Ends Between"), and
Fall (PUT / Rise-Fall down-side).

Why this matters for RDBEAR specifically:
    - RDBEAR (like RDBULL) only accepts MINUTE durations for these contracts on
      Deriv - no tick durations - which is exactly the 2-60 min range analyzed
      here.
    - RDBEAR is a synthetic index built with a structural drift (it trends by
      construction), so every one of these contract types lives or dies on how
      well you estimate that drift and its volatility-scaling behaviour across
      different minute-durations - that's what this script measures directly
      from real historical + live ticks, rather than assuming.

Why NOTOUCH, EXPIRYRANGE, and Fall each need their OWN table (not one shared
table relabeled):
    - NOTOUCH asks: does price ever wander outside the barrier at ANY point
      during the window? -> needs the running max/min of the path.
    - EXPIRYRANGE asks: where does price LAND at expiry, ignoring what it did
      in between? -> needs only the window's endpoint, band-relative.
    - Fall asks: is price lower at expiry than at entry? There is no barrier
      at all - it's a pure directional bet, so what matters is the probability
      itself AND whether that probability is statistically distinguishable
      from 50/50, which is the real risk at short durations on a drifting
      instrument (a real edge can look identical to noise until you check).
...
What this script does (no trading, no proposals, no buys - pure data/analysis):
    ...
    4. Computes, from the real tick series:
         ...
         - NOTOUCH table: for each candidate duration in
           {2,3,5,10,15,20,30,45,60} minutes and barrier width in
           {0.5,1.0,1.5,2.0,2.5,3.0} x sigma, the empirical fraction of real
           historical windows that ever touched a barrier that wide, up-side
           and down-side separately.
         - EXPIRYRANGE table: same duration/width grid, but the empirical
           fraction of windows that END inside a +/-width band around the
           entry price (touching outside the band mid-window doesn't matter,
           only the landing spot does).
         - Fall table: per duration, the empirical probability price is lower
           at expiry than at entry, computed two ways - (a) over ALL
           overlapping windows (more data, biased toward looking significant
           because windows share ticks and aren't independent), and (b) over
           only NON-overlapping windows (fewer samples, but a legitimate
           z-test against 50/50 that isn't fooled by autocorrelation). Both
           the edge size and the honest p-value are reported side by side.
    5. Prints/saves a summary report (JSON) you can use to pick sane
       (contract type, duration, barrier) defaults for live bots, and
       re-prints the report every REPORT_EVERY_MINUTES while it keeps
       collecting.
...
Environment variables:
    ...
    COLLECT_MINUTES       how long to keep collecting live ticks (default: 60;
                          set to 0 to run until Ctrl+C - recommended on Railway)
    REPORT_EVERY_MINUTES  how often to re-print the analysis while collecting
                          (default: 5)
    DATA_DIR              directory for the CSV/JSON outputs (default: ".").
                          On Railway, point this at a mounted Volume so data
                          survives redeploys - the base container filesystem
                          is ephemeral.

Run:
    python rdbear_multi_explorer.py
"""
...
# Duration/barrier grid this analysis targets: 2-60 minutes (RDBEAR has no
# tick-duration option for these contracts), representative candidates across
# that range. Shared across NOTOUCH (barrier width) and EXPIRYRANGE (band
# half-width); Fall uses only the duration grid since it has no barrier.
DURATION_GRID_MIN = (2, 3, 5, 10, 15, 20, 30, 45, 60)
BARRIER_SIGMA_GRID = (0.5, 1.0, 1.5, 2.0, 2.5, 3.0)

# DATA_DIR lets you point outputs at a mounted Railway Volume (recommended -
# Railway's default container filesystem is ephemeral and is wiped on every
# redeploy, so without a volume you lose the CSV/JSON on each new deploy).
DATA_DIR = os.getenv("DATA_DIR", ".")
os.makedirs(DATA_DIR, exist_ok=True)
TICK_CSV_PATH = os.path.join(DATA_DIR, f"{SYMBOL.lower()}_ticks.csv")
REPORT_JSON_PATH = os.path.join(DATA_DIR, f"{SYMBOL.lower()}_multi_contract_analysis.json")
...
def _ztest_proportion(p_hat: float, n: int, p0: float = 0.5) -> tuple[float, float]:
    """Two-sided z-test of a sample proportion against p0 (default 50/50).
    Returns (z, p_value). Caller is responsible for feeding an n that reflects
    genuinely independent samples - see the non-overlapping-window logic in
    analyze_contracts, since overlapping windows share ticks and would make
    this test overconfident if n were the raw overlapping-window count.
    """
    if n <= 1:
        return 0.0, 1.0
...
def analyze_contracts(prices: np.ndarray, tick_rate: float,
                       duration_grid_min=DURATION_GRID_MIN,
                       sigma_grid=BARRIER_SIGMA_GRID) -> dict:
    """For each duration, slides a window of that length (converted to ticks
    via the live-measured tick rate) over the REAL historical price path ONCE,
    then asks three different questions of the same windows - one per contract
    family, since each cares about a different feature of the path:

      NOTOUCH      - running max/min of the path vs a barrier (touched at
                     ANY point?), up-side and down-side separately.
      EXPIRYRANGE  - only the window's ENDPOINT vs a band (landed inside?),
                     what happens mid-window is irrelevant.
      Fall (PUT)   - only the SIGN of the endpoint move (down vs up), no
                     barrier at all, reported both as a raw empirical
                     probability (overlapping windows - more data, but
                     autocorrelated) and as a properly significance-tested
                     probability (non-overlapping windows - fewer samples,
                     but an honest z-test against 50/50).
    ...
    log_p = np.log(prices)
    n = len(log_p)
    notouch_rows: list[dict] = []
    expiryrange_rows: list[dict] = []
    fall_rows: list[dict] = []

    local_returns = np.diff(log_p)
    sigma_tick_global = np.std(local_returns) if len(local_returns) > 1 else 1e-6
    ...
    for dur_min in duration_grid_min:
        ...
        windows = np.lib.stride_tricks.as_strided(
            log_p[: n_windows + window_ticks], shape=shape, strides=strides
        )
        rel = windows - starts[:, None]
        win_max = rel.max(axis=1)
        win_min = rel.min(axis=1)
        win_end = rel[:, -1]   # position at (approx.) expiry, relative to entry

        sigma_window = sigma_tick_global * math.sqrt(window_ticks)

        # ---------------- NOTOUCH: did the path ever touch the barrier? -----
        for k in sigma_grid:
            dist = k * sigma_window
        ...
        # ---------------- EXPIRYRANGE: did it LAND inside the band? ---------
        for k in sigma_grid:
            dist = k * sigma_window
            end_inside = np.mean(np.abs(win_end) <= dist)
            expiryrange_rows.append({
                "duration_min": dur_min,
                "window_ticks": window_ticks,
                "sigma_k": k,
                "band_halfwidth_log": round(float(dist), 6),
                "p_end_inside": round(float(end_inside), 4),
                "p_end_outside": round(float(1 - end_inside), 4),
                "n_windows_sampled": int(n_windows),
            })

        # ---------------- Fall: directional, no barrier ----------------------
        p_down_overlap = float(np.mean(win_end < 0))
        p_up_overlap = float(np.mean(win_end > 0))
        p_flat_overlap = float(np.mean(win_end == 0))

        # Non-overlapping (independent) windows for an honest significance
        # test - stride by window_ticks so no two windows share a tick.
        stride = window_ticks
        n_indep = n_windows // stride if stride > 0 else 0
        if n_indep >= 2:
            idx = np.arange(0, n_indep * stride, stride)
            indep_end = win_end[idx]
            p_down_indep = float(np.mean(indep_end < 0))
        else:
            n_indep = max(n_indep, 1)
            p_down_indep = p_down_overlap

        z, p_value = _ztest_proportion(p_down_indep, n_indep, p0=0.5)

        fall_rows.append({
            "duration_min": dur_min,
            "window_ticks": window_ticks,
            "p_fall_overlapping": round(p_down_overlap, 4),
            "p_rise_overlapping": round(p_up_overlap, 4),
            "p_flat_overlapping": round(p_flat_overlap, 4),
            "edge_vs_50_50_overlapping": round(p_down_overlap - 0.5, 4),
            "n_windows_sampled_overlapping": int(n_windows),
            "p_fall_independent_windows": round(p_down_indep, 4),
            "n_independent_windows": int(n_indep),
            "z_score_independent": round(z, 3),
            "p_value_independent": round(p_value, 4),
            "significant_at_5pct": bool(p_value < 0.05),
        })

    return {
        "notouch_table_2to60min": notouch_rows,
        "expiryrange_table_2to60min": expiryrange_rows,
        "fall_directional_table_2to60min": fall_rows,
    }
...
def build_report(store: TickStore) -> dict:
    ...
    contract_tables = analyze_contracts(prices, tick_rate)
...
def print_report_summary(report: dict):
    ...
    print("-" * 88)
    print("  NOTOUCH  (touched barrier at ANY point during the window?)")
    print(f"  {'dur(min)':>8} {'sigma':>6} {'barrier(log)':>13} "
          f"{'P(notouch up)':>14} {'P(notouch down)':>16}")
    for row in report["notouch_table_2to60min"]:
    ...
    print("-" * 88)
    print("  EXPIRYRANGE  (landed inside the band AT EXPIRY - mid-window excursions don't matter)")
    print(f"  {'dur(min)':>8} {'sigma':>6} {'band(log)':>13} "
          f"{'P(ends inside)':>15} {'P(ends outside)':>16}")
    for row in report["expiryrange_table_2to60min"]:
        print(f"  {row['duration_min']:>8} {row['sigma_k']:>6.1f} "
              f"{row['band_halfwidth_log']:>13.5f} "
              f"{row['p_end_inside']:>15.3f} {row['p_end_outside']:>16.3f}")

    print("-" * 88)
    print("  Fall / PUT  (directional only, no barrier - is the edge over 50/50 statistically real?)")
    print(f"  {'dur(min)':>8} {'P(fall) all':>12} {'P(fall) indep':>14} "
          f"{'n indep':>8} {'z':>7} {'p-value':>9} {'sig@5%':>7}")
    for row in report["fall_directional_table_2to60min"]:
        print(f"  {row['duration_min']:>8} {row['p_fall_overlapping']:>12.3f} "
              f"{row['p_fall_independent_windows']:>14.3f} "
              f"{row['n_independent_windows']:>8} {row['z_score_independent']:>7.2f} "
              f"{row['p_value_independent']:>9.4f} "
              f"{('YES' if row['significant_at_5pct'] else 'no'):>7}")
    print("=" * 88 + "\n")
